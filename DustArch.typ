...
To be able to boot from a `BIOS`, as well as a `UEFI` system, simply follow both
of these guides.

==== `BIOS`

For installing `limine` on a `BIOS` system, you first need to copy #filepath("/usr/share/limine/limine.sys") (which
replaces the need for a boot partition, like `grub` uses it) to a #filepath("/") or #filepath("/boot") directory
of any partition on the disk you want to try and boot from.
...
Simply copy #filepath("/usr/share/limine/BOOTX64.EFI") to the appropriate
location on your `EFI System Partition`
...
#note[
In case you're using the #link("*Secure Boot")[Secure Boot] method described in #link("*`PreLoader`"),
you would need to name it `loader.efi`, as the `PreLoader` takes the place of
the `BOOTX64.EFI` which gets auto started by most `UEFI` systems.
]

==== config

The only thing left to do is to create a `limine.cfg` file with all your desired
boot entries in it.
...
First off we'll define a variable which we then use throughout our boot entries,
in order to reduce complexity and redundancy and increase readability.

#note[
You need to replace the `[...]` part with the appropriate values for your
system.
...
#filesrc(part: true, perm: true, "/boot/limine.cfg")[
```
TIMEOUT=no
...
First off, we have some binaries to be present in the image, so that if we drop
into a recovery shell, we can use them.

#filesrc(part: true, perm: true, "/etc/mkinitcpio.conf")[
```
...
In order to use the hibernation feature, you should make sure that your `swap` partition/file
is at least the size of your RAM.

If you use a `busybox` based `ramdisk`, you need to add the `resume` hook to #filepath("/etc/mkinitcpio.conf"),
before `fsck` and definitely after `block`

#note[
When using `EFISTUB` without `sbupdate`, your motherboard has to support kernel
parameters for boot entries. If your motherboard doesn't support this, you would
need to use some other kind of boot manager (see @boot-manager).
]

=== `HOOKS`

Now we will specify every hook we need. Mention-worthy additions to the default
...
#pkgtable(extra: "terminus-font", aur: "mkinitcpio-colors-git")

By creating a file #filepath("/etc/vconsole.conf") we can specify a custom font
and color scheme to use

#filesrc(perm: true, "/etc/vconsole.conf")[
```
KEYMAP=us
FONT=ter-116n
COLOR_0=282a36
...
#caution[
I think it is worth noting that lately, I didn't use a `systemd` based `ramdisk` on
my portable setup anymore, as I encountered some issues.

The underlying issue apparently was having the `block` and `keyboard` hooks
located after the `autodetect` hook. Reversing this so that `block` and `keyboard` precedes `autodetect` seems
to fix the issue. In any case, the `fallback initramfs` should always work.

It is worth noting though, that with the `busybox` based one, you lose the
ability to unlock multiple `LUKS` encrypted partitions/devices at once if they
share the same password. In that case, you must use the #filepath("/etc/crypttab").
]

There is nothing particularly better about using a `systemd` based `ramdisk` instead
of a `busybox` one; it's just that I prefer it.

Some advantages, at least in my opinion, that the `systemd` based `ramdisk` has,
are the included `resume` hook, as well as password caching, when decrypting
encrypted volumes, which means that because I use the same `LUKS` password for
both my data storage `HDD`, as well as my `cryptroot`, I only have to input the
password once for my `cryptroot` and my data storage `HDD` will get decrypted
too, without the need to create #filepath("/etc/crypttab") entries, etc.

To switch to a `systemd` based `ramdisk`, you will normally need to substitute
the `busybox` specific hooks for `systemd` ones. You will also need to use `systemd` hooks
from now on, for example, `sd-encrypt` instead of `encrypt`.
...
This way of handling secure boot aims at just making everything work! It is not
how Secure Boot was intended to be used, and you might as well disable it.

If you need Secure Boot to be enabled, e.g. for Windows, but you couldn't care
less for the security it could bring to your device, or if you want to use this
installation on multiple systems, where Secure Boot could be enabled, use this
method.

If you want to actually make use of the Secure Boot feature, read
@manual-secure-boot.

I know I told you you're ready to boot into your new system. That is only
correct if you're _not_ using Secure Boot. You can either disable Secure Boot in
your firmware settings or use `PreLoader` as a pre-bootloader.

If you decide to use Secure Boot, you must first install the package. Now we
need to copy the `PreLoader` and the `HashTool`, which gets launched if the hash
of the binary that is to be loaded (`loader.efi`) is not registered in the
firmware yet, to our `EFI System Partition`
...
#note[
If you have to use `bcdedit` from within Windows, as explained in the section #link("*`grub` - `UEFI`"),
...
Just choose "Enroll Hash", choose the appropriate `loader.efi`, and enroll the
...
As this is a very tedious and time-consuming process, it only makes sense when
also utilizing some disk encryption, which is why I would advise you to read
first.
...
To allow deletion of the `PK` for different firmwares that don't provide this
functionality out of the box, we have to sign an empty file.
...
As your plan is to be able to control which things do boot on your system and
which don't, you're going through all this hassle to create and enroll custom
keys, so only `EFI` binaries signed with said keys can be executed.
...
To ensure a smooth operation with actual security, we need to move some stuff
around.

===== Kernel, `initramfs`, microcode

`pacman` will put its unsigned and unencrypted kernel, `initramfs` and microcode
images into #filepath("/boot"), which is why it is no longer a good idea to
leave your `EFI System Partition` mounted there. Instead, we will create a new
mount point under #filepath("/efi") and modify our `fstab` accordingly.

===== Keys

As you probably want to automate signing sooner or later and only use the
ultimately necessary keys for this process and store the other more critical
keys somewhere safer and more secure than your `root` home directory, we will
move the necessary ones.

I like creating a #filepath("/etc/efi-keys") directory, `chmod`ded to `700`, and
placing my `db.crt` and `db.key` there. All the keys will get packed into a `tar` archive,
encrypted with a robust symmetric passphrase, and stored somewhere secure and
safe.

==== Signing

Signing is the process of signing your `EFI` binaries for them to be allowed to
be executed by the motherboard firmware. At the end of the day, that's why
you're doing all this, to prevent an attack by launching unknown code.

===== Manual signing

Of course, you can sign images yourself manually. In my case, I used this to
sign the boot loader, kernel, and `initramfs` of my USB installation of Arch
Linux.

As always, manual signing also comes with its caveats!

If I update my kernel, boot loader, or create an updated `initramfs` on my Arch
Linux USB installation, I must sign those files again to boot it on my PC.

Of course, you can always script and automate stuff, but if you want something
easier for day-to-day use, I recommend trying out `sbupdate`, which I will
explain in the next paragraph @sbupdate.

For example, if I want to sign the kernel image of my USB installation, where I
mounted the boot partition to #filepath("/mnt/DustPortable/boot"), I must do the
following.
...
Of course, if you're using Secure Boot productively, you would want something
more practical than manual signing, especially since you need to sign
...
It not only signs everything and also foreign `EFI` binaries, if specified, but
also combines your kernel and `initramfs` into a single executable `EFI` binary,
so you don't even need a boot loader if your motherboard implementation supports
booting those.

After installing `sbupdate`, we can edit the #filepath("/etc/sbupdate.conf") file
...
`sbupdate` will be executed upon kernel updates by `pacman`, but not if you
change your `initramfs` with something like `mkinitcpio`. In that case, you will
have to run `sbupdate` manually.

==== Add `EFI` entries

#pkgtable(core: "efibootmgr")

Now the only thing left to do, if you want to stay boot loader free, is to add
the signed images to your `NVRAM` boot list. You can do this with `efibootmgr`.
...
First off, copy all `.cer`, `.esl`, and `.auth` files to a `FAT` formatted
filesystem. I'm using my `EFI System Partition` for this.

After that reboot into the firmware setup of your motherboard, clear the
existing Platform Key, to set the firmware into "Setup Mode" and enroll the `db`, `KEK`,
and `PK` certificates in sequence.

Enroll the Platform Key last, as it sets most firmware's Secure Boot sections
back into "User mode", exiting "Setup Mode".

= Inside the `DustArch`

This section helps at setting up the customized system from within an installed
system.

This section mainly provides aid with the basic setup tasks, like networking,
dotfiles, etc.

Not everything in this section is mandatory.

This section is rather a guideline because it is easy to forget some steps
needed (for example `jack` for audio production), which only become apparent
when they're needed or stuff fails.

It is furthermore the responsibility of the reader to decide which steps to skip
and which need further research. As I mentioned, this is only a guide and not
the answer to everything. So reader discretion is advised!
...
If there is no connection and you're indeed using a wired connection, try
...
NetworkManager's command line tool, or `nmtui`, the NetworkManager terminal user
interface, to connect to a Wi-Fi network.

I never got `nmtui` to behave like I wanted it to, in my particular case at
least, which is the reason why I use `nmcli` or the GUI tools.

First make sure, the scanning of nearby Wi-Fi networks is enabled for your Wi-Fi
device
...
After making sure that you have a working Internet connection, you can then
proceed to update and upgrade all installed packages by issuing

#terminal-root("~")[`pacman -Syu`]

== Enabling the `multilib` repository

In order to make 32-bit packages available to `pacman`, we'll need to enable the `multilib` repository
in #filepath("/etc/pacman.conf") first. Simply uncomment

#filesrc(part: true, perm: true, "/etc/pacman.conf")[
```
[multilib]
...
I am using `nushell` because of its nice functionality and because I'm a sucker
for `rust` software.

If you remember correctly, we set the login shell to `bash` when creating the `dustvoice` user,
so you might wonder why we didn't directly set it to `nu`. Well `nushell` isn't
completely `POSIX` compliant, and neither does it want to be. Therefore running `nu` as
a login shell might not be the absolute best experience you ever had.

Instead, we populate our `.bashrc` with some scripting that will let `nu` take
over any _interactive_ shell, while scripts, etc. that expect a `POSIX` compliant
shell can have their way.

#note[
You can replicate the following instructions directly for the `root` user, to
get the same kind of experience there
]
...
Don't worry about the looks, by the way, we're gonna change all that in just a
second.
...
Install the package and you're good to go for now, as we'll care about the `.gitconfig` in
just a second.
...
If you've followed the tutorial using a recent version of the archiso, you'll
probably already have the most recent version of `gnupg` installed by default.

=== Smartcard shenanigans

#pkgtable(extra: "ccid libusb-compat opensc pcsclite usbip")

After that, you'll still have to set up `gnupg` correctly. In my case I have my
private keys stored on a smartcard.

To use it, I'll have to install the listed packages and then enable and start
the `pcscd.service` service
...
If your smartcard still isn't detected, try logging off completely or even
restarting, as that sometimes is the solution to the problem.
...
the required packages beforehand

This will ensure, we proceed through the following section without the need for
interruption because a package needs to be installed so that the following
content can be condensed to the relevant information.

== Setting up a `home` environment <home_setup>

In this step we're going to set up a home environment for both the `root` and my
personal `dustvoice` user.

In my case, these 2 home environments are mostly equivalent, which is why I'll
execute the following commands as the `dustvoice` user first and then switch to
the `root` user and repeat the same commands.

I decided on this, as I want to edit files with elevated permissions and still
have the same editor style and functions/plugins.

Note that this comes with some drawbacks. For example, if I change a
configuration for my `dustvoice` user, I would have to update it for the `root` user
too regularly.

Also, I have to register my smart card for the root user. This in turn is
problematic, because the `gpg-agent` used for `ssh` authentication, doesn't
behave well when used within a #cmd[`su`] or #cmd[`sudo -i`] session. So in
order to update `root`'s config files I would either need to symlink everything,
which I won't do, or I'll need to log in as the `root` user now and then, to
update everything.

In my case, I want to access all my `git` repositories with my `gpg` key on my
smartcard. For that, I have to configure the `gpg-agent` with some configuration
files that reside in a `git` repository. This means I will have to get along
with using the `https` URL of the repository first and later changing the URL
either in the corresponding `.git/config` file or by issuing the appropriate
command.

=== Use `dotfiles` for a base config

To provide myself with a base configuration, which I can then extend, I maintain
a `dotfiles` repository (on my self-hosted GitLab instance, mirrored of course),
which contains all kinds of configurations.

The special thing about this `dotfiles` repository is that it _is_ my home
folder. By using a curated `.gitignore` file, I'm able only to include the
configuration files I want to keep between installs into the repository and
ignore everything else.

To achieve this very specific setup, I have to turn my home directory into said `dotfiles` repository
first
...
#important[
This has lead to some problems in the past. Generally I would try and use the
new #cmd[`git switch`] command like so:
...
Furthermore, I want to use my `nvim` repository, which contains all my
configurations and plugins for `neovim`, on Windows, but without all the
Linux-specific configuration files. I also use the `Pass` repository on my
Android phone and Windows PC, where I only need this repository without the
other Linux configuration files.

Before we are able to update the `submodule`s (`nvim` config files and `pass`)
though, we will have to set up our `gpg` key as an `ssh` key, as I use it to
authenticate
...
You would have to adapt the `keygrip` present in the #filepath("~/.gnupg/sshcontrol") file
to your specific `keygrip`, retrieved with #cmd[`gpg -K --with-keygrip`].

#important[
If you're inside a VM, you of course need to pass the smartcard somehow to said
VM.

#pkgtable(extra: "usbip")

If you're inside a `Hyper-V` VM, you need to utilize `usbip`. If you're using `fish`,
there's a script under #filepath("~/.config/fish/usbip-man.fish")
]

Now, as mentioned before, I'll switch to using `ssh` for authentication, rather
than `https`
...
as the `gpg-agent` behave properly, is to re-login. We'll do just that

#terminal("~")[`exit`]

It is very important to note, that I mean _a real re-login_.

That means that if you've used `ssh` to log into your machine, it probably won't
be sufficient to login into a new `ssh` session. You may need to restart the
machine entirely.
...
complete: `gpg-agent` forwarding over `ssh`. This is very important for me, as I
want to use my smartcard on my development server too, which requires me, to
forward/tunnel my `gpg-agent` to my remote machine.

First of all, I want to set up a config file for `ssh`, as I don't want to pass
all parameters manually to ssh every time.
...
#filesrc(part: true, perm: true, "/etc/sshd/config")[
```
...
If you use `alacritty`, to connect to your remote machine over `ssh`, you will
need to install the `alacritty` package on the remote machine too, as `alacritty` uses
its own `$TERM`.
...
As mentioned before, you would now switch to the `root` user, either by logging
in as `root`, or by using

#terminal-root("~")[`-iu root`]

Now go back to @home_setup to repeat all commands for the `root` user.

A native login would be better compared to #cmd[`sudo -iu root`], as there could
be some complications, like already running `gpg-agent` instances, etc., which
you would need to manually resolve when using #cmd[`sudo -iu root`].
...
#filesrc(part: true, perm: true, "/etc/asound.conf")[
```
defaults.pcm.card 2
defaults.ctl.card 2
```
...
It should be clear, that you would have to switch out `2` with the number
corresponding to the sound card you want to use.

=== `pulseaudio`

#pkgtable(extra: "pavucontrol pulseaudio pulsemixer")

Some applications require `pulseaudio`, or work better with it, for example, `discord`,
so it might make sense to use `pulseaudio` (although #link("*`pipewire`")[`pipewire`] could
replace it).

For enabling real-time priority for `pulseaudio` on Arch Linux, please make sure
your user is part of the `audio` group and edit the file #filepath("/etc/pulse/daemon.conf"),
so that you uncomment the lines

#filesrc(part: true, perm: true, "/etc/pulse/daemon.conf")[
```
high-priority = yes
nice-level = -11
...
If your system can handle the load, you can also increase the remixing quality,
by changing the `resample-method`
...
Of course, a restart of the `pulseaudio` daemon is necessary to reflect the
...
#pkgtable(extra: "cadence jack2 pulseaudio-jack")

If you either want to manually control audio routing or if you use some kind of
audio application like `ardour`, you'll probably want to use `jack` and then `cadence` as
a GUI to control it, as it has native support for bridging `pulseaudio` to `jack`.
...
#tip[
If you don't want to have conflicts, you need to stop `pulseaudio.service` and
...
#pkgtable(extra: "libao libid3tag libmad libpulse opus sox twolame wavpack")
...
#pkgtable(extra: "blueman bluez bluez-utils pulseaudio-bluetooth")

To set up Bluetooth, we need to install the `bluez` and `bluez-utils` packages
to have at least a command line utility `bluetoothctl` to configure connections.
...
To use `bluetoothctl` and access your PC's Bluetooth device, your user must be a
...
In most cases, your Bluetooth interface will be preselected and defaulted, but
in some cases, you might need to first select the Bluetooth controller
...
You can turn device discovery mode off again after your desired device has been
found
...
If your device doesn't support PIN verification, you might need to trust the
device manually
...
If you decide that you want to use a graphical desktop environment, you have to
install additional packages for that to work.
...
required for `neovim`'s clipboard to work correctly. It is not required, though.

==== NVIDIA

#pkgtable(extra: "nvidia nvidia-utils nvidia-settings opencl-nvidia")

If you also want to utilize special NVIDIA functionality, for example for `davinci-resolve`,
you'll most likely need to install their proprietary driver.
...
will enable you to _"Save to X Configuration File"_, which merges your changes
with #filepath("/etc/X11/xorg.conf").

With

#terminal("~")[`nvidia-settings`]

you'll only be able to save the current configuration to #filepath("~/.nvidia-settings-rc"),
which you have to source after `X` startup with

#terminal("~")[`nvidia-settings --load-config-only`]

You will have to reboot sooner or later after installing the NVIDIA drivers, so
you might as well do it now before any complications come up.
...
#pkg-aur("nvidia-xrun") instead of `startx`. This has the advantage, of the `nvidia` kernel
modules, as well as the `nouveau` ones not loaded at boot time, thus saving
power. #pkg-aur("nvidia-xrun") will then load the correct kernel modules, and
run the `.nvidia-xinitrc` script in your home directory (for more file locations
look into the documentation for
#pkg-aur("nvidia-xrun")).

At the time of writing, #pkg-aur("nvidia-xrun") needs `sudo` permissions before
executing its task.
...
directory is located. There are 2 possible locations, generally speaking: #filepath("/etc/modprobe.d") and #filepath("/usr/lib/modprobe.d").
In my case, it was the latter, which I could tell, because this directory
already had files in it.

Now I'll create a new file named `nvidia-xrun.conf` and write the following into
it
...
should return no output. Else you might have to place some additional entries
into the file.

Of course, you'll need to reboot, after blacklisting the modules and before
issuing the 2 commands mentioned.
...
The required `.nvidia-xinitrc` file, mentioned previously, should already be
provided in the `dotfiles` repository.
...
Things behave a little different with `wayland`. But fear not!

As I'm using `sway` as my `wayland` compositor and it almost is a drop-in
replacement for `i3`, you shouldn't be long from `GUI`'ing away.
...
I would recommend installing `tmux` which enables you to have multiple terminal
instances (called `windows` in `tmux`) open at the same time. This makes working
with the linux terminal much easier.
...
Life is all about communicating. Here are some pieces of software to do exactly
that.
...
`weechat` is an `IRC` client for the terminal, with the best features and even a `vim` mode,
by using a plugin
...
You might need to authenticate with `NickServ`, before being able to write in a
channel
...
after that, using the cursor, select the entry you want to modify (for example `plugins.var.python.vimode`)
and then press `s` (make sure you're in `insert` mode) and `Return`, in order to
...
To use `asciidoctor-pdf`, you might be wondering how you are supposed to open
the generated PDFs from the native linux fish.

This `fbida` package provides the `fbgs` software, which renders a PDF document
...
Some additional software provides some kind of `GUI` to work with, but that can
be useful in a `console` only environment nevertheless.

=== `Pass`word management

I'm using `pass` as my password manager. As we already installed it in the step
and updated the `submodule` that holds our `.password-store`, there is nothing
left to do in this step
...
In order to use `asciidoctor`, we have to install `ruby` and `rubygems`. After
that, we can install `asciidoctor` and all its required gems.

If you want to have pretty and highlighted source code, you'll need to install a
code-formatter too.
...
Now the only thing left, in my case at least, is adding #filepath("~/.gem/ruby/2.7.0/bin") to
your path.

Please note that if you run a ruby version different from `2.7.0`, or if you
upgrade your ruby version, you have to use the `bin` path for that version.
...
which then gets sourced by the provided `.zshenv` file. An example is provided
with the `.zshpath.example` file

You might have to re-#cmd[`source`] the `.zshenv` file to make the changes take
effect immediately

#terminal("~")[`source .zshenv`]

If you want to add a new entry to the `path` variable, you have to append it to
the array
...
If you use another shell than `zsh`, you might have to do something different,
...
Note that apparently in the new `JUCE` version, `cmake` support is integrated.
It remains to be seen how well this will work and if `FRUT`
will become obsolete.

The information in this guide should be updated ASAP if it is apparent that `FRUT` has
now become obsolete.
...
where `ladspa` and `lib32-freeglut` are not necessarily needed.

=== Additional development tools

Here are just some examples of development tools one could install in addition
...
We already have `clang-format` as a code-formatter, but this only works for `C`-family
languages. For `java` stuff, we can use `astyle`
...
#pkgtable(extra: "gvfs-mtp libmtp")

Now you should be able to see your phone inside either your preferred
filemanager, in my case `thunar`, or #pkg-aur("gigolo").

If you want to access the android's file system from the command line, you will
need to either install and use #pkg-aur("simple-mtpfs"), or `adb`
...
#filesrc(part: true, perm: true, "/etc/fuse.conf")[
```
user_allow_other
```
]
...
If the server is currently not running, #cmd[`adb`] will output an error with a `Connection refused` message.

Now connect your phone, unlock it, and start the `adb` server

#terminal("~")[`adb start-server`]

If the PC is unknown to the Android device, it will display a confirmation
dialog. Accept it and ensure that the device was recognized
...
Of course, you would need to have the _developer options_ unlocked, as well as
the _USB debugging_ option enabled within them, for `adb` to even work.

=== Partition management

#pkgtable(extra: "gparted parted")

You may also choose to use a graphical partitioning software instead of
`fdisk` or `cfdisk`. For that, you can use `gparted`. Of course, there is also
the `console` equivalent `parted`.

=== PDF viewer

#pkgtable(extra: "evince zathura zathura-pdf-mupdf")

To use `asciidoctor-pdf`, you might be wondering how you are supposed to open
the generated PDFs using the GUI.

The software `zathura` has a minimalistic design and UI with a focus on vim
keybinding, whereas `evince` is a more desktop-like experience, with things like
a print dialogue, etc.

=== Process management

#pkgtable(extra: "htop xfce4-taskmanager")

The native tool is `top`.

The next evolutionary step would be `htop`, which is an improved version of `top` (like `vi` and `vim` for
example)
...
As you now have a working graphical desktop environment, you might want to
install some software to utilize your newly gained power.

=== Session Lock

#pkgtable(extra: "xsecurelock xss-lock")

Probably the first thing you'll want to set up is a session locker, which locks
your `X`-session after resuming from sleep, hibernation, etc. It then requires
you to input your password again, so no unauthorized user can access your
machine.

I'll use `xss-lock` to hook into the necessary `systemd` events and then use `xsecurelock` as
my locker.

You need to make sure this command gets executed upon the start of the
`X`-session, so hook it into your window manager startup script, or in a file
called by your desktop environment
...
In order for GUI applications to acquire `sudo` permissions, we need to install
a `PolicyKit` authentication agent.

We could use `gnome-polkit` for that purpose, which resides inside the official
repositories, but I decided on using #pkg-aur("xfce-polkit").

Now you just need to start up #pkg-aur("xfce-polkit") before trying to execute
something like `gparted` and you'll be prompted for your password.

As I already launch it as a part of my `bspwm` configuration, I won't have to
worry about that.
...
You might want to consider installing `nitrogen`, in order to be able to set a
background image
...
`brave` without screen tearing, you might want to consider using a compositor,
in my case one named `picom`
...
To install the `NetworkManager` applet, which lives in your tray and provides
you with a quick method to connect to different networks, you have to install
the `network-manager-applet` package

Now you can start the applet with

#terminal("~")[`nm-applet &`]

If you want to edit the network connections with a more fullscreen approach, you
can also launch #cmd[`nm-connection-editor`].

The `nm-connection-editor` doesn't search for available Wi-Fi. You would have to
set up a Wi-Fi connection completely by hand, which could be desirable depending
on how difficult it is to set up your Wi-Fi.

=== Show keyboard layout

#pkgtable(aur: "xkblayout-state")

To show, which keyboard layout and variant is currently in use, you can use #pkg-aur("xkblayout-state")
...
#pkgtable(extra: "gvfs thunar", aur: "gigolo")
...
`sudo`, and in order to have a GUI for mounting stuff, you would need to use #pkg-aur("gigolo") and `gvfs`.

=== Archive manager

#pkgtable(extra: "cpio unrar unzip xarchiver zip")

As we now have a file manager, it might be annoying, to open up a terminal every
time you simply want to extract an archive of some sort. That's why we'll use `xarchiver`.

=== Web browser

#pkgtable(extra: "browserpass firefox firefox-i18n-en-us")
...
Although this shouldn't be your only precaution, using
...
#pkgtable(aur: "obs-linuxbrowser-bin obs-glcapture-git obs-studio-git")
...
In order to show the viewers what keystrokes you're pressing, you can use
something like #pkg-aur("screenkey")

For ideal use with `obs`, my `dotfiles` repository already provides you with the #cmd[`screenkey-obs`] alias
for you to run with `zsh`.
...
allocate most of the RAM, which is why I added the following line to the file

#filesrc(part: true, perm: true, "/etc/security/limits.conf")[
```
...
To e.g. edit and produce audio, you could use `ardour` because it's easy to use,
stable, and cross-platform.

#pkgtable(extra: "ffmpeg")

Ardour won't natively save in the `mp3` format, due to licensing stuff. In order
to create `mp3` files, for sharing with other devices, because they have
problems with `wav` files, for example, you can just use
`ffmpeg`.
...
Instead of `ardour`, I'm using `reaper`, which is available for linux as a beta
version, in my case more stable than `ardour` and easier to use for me.

=== Virtualization

#pkgtable(extra: "virtualbox virtualbox-host-modules-arch")

You might need to run another OS, for example, Mac OS, from within Linux, e.g.
for development/testing purposes. For that, you can use
`virtualbox`.
...
The second option would be `lutris`, a program, that configures a wine instance
correctly, etc.

=== Wacom

#pkgtable(extra: "libwacom xf86-input-wacom")
...
In order to connect to a machine over `VNC` or to connect to a machine using the `Remote Desktop Protocol`,
for example, to connect to a Windows machine, I'll need to install #pkg-aur("freerdp"),
as well as
`libvncserver`, for `RDP` and `VNC` functionality respectively, as well as `remmina`,
to have a GUI client for those two protocols.
...
You have to make sure, _that your boot partition is mounted at #filepath("/boot")_ in
order for everything to upgrade correctly. That's because the moment you upgrade
the `linux` package without having the correct partition mounted at #filepath("/boot"),
your system won't boot. You also might have to do #cmd[`grub-mkconfig -o /boot/grub/grub.cfg`] after
you install a different kernel image.

If your system _indeed doesn't boot_ and _boots to a recovery fish_, then double
check that the issue really is the not perfectly executed kernel update by
issuing
...
the process will likely create some unwanted files. The process will also create
a new #filepath("/boot") folder, which we're going to delete afterward.
...
There should be a file located named something like `linux-<version>.pkg.tar.xz`,
where `<version>` would be somewhat equivalent to the previously noted version
number
...
This documentation is structured in a way that allows you to keep a printed
version up to date without the need to reprint the whole thing. This is why
every section starts on a new page and page numbers are omitted.

In the following sections, I will quickly describe the meaning of a handfull of
special markup, which hopefully makes the whole document more readable and
structured.

== Blocks

There are special blocks that are supposed to draw your attention and seperate
their contents:
...
Terms denoted by a `monospaced font` are mostly commands/programs or specific
terms that are generally accepted to be a universal name for what they describe.

For example when I say that you need to `cd` into the directory, `cd` denotes
the program/command I intend you to use. If I say you need to read the `PKGBUILD` or
that you need to create an `EFI System Partition`, that in turn is the
universally accepted name for both of those things.

#note[
Note however that in case of the mentioned programs, _`cd` into the direcory_,
or reading the _`PKGBUILD`_, this is just the lowest level / highest abstraction
...
Normally, you would simply embed a #link("https://archlinux.org")[link] into the
document.

But a reader of this guide who uses `Dead-Tree-Format`#emoji.tm aka. paper, will
neither be able to distinguish the link from the rest of the text, nor be able
to get any information regarding the target of the link.

This is why I try and provide every link as a #linkfn("https://archlinux.org")[footnote-link],
which works exactly like a regular link with the big difference that the link
target is fully written out in the footnotes of the page.

#note[
The footnote numbering resets for each page, to ensure the `hot-swap`-ability of
the pages as mentioned in @glossary.
]
...
If a package is simply mentioned within a paragprah, e.g. #pkg("alacritty"), it
will be visually separated from the rest of the content and a link provided in
the footnotes (see @links).

This way you can also easily tell a normal package #pkg("alacritty") apart from
an `AUR` package #pkg-aur("sbupdate-git"). Both the packge name and the
footnote-link are clickable and lead you to the generated URL.
#pagebreak()
The second option is a package-table which categorizes the packages into
repositories.
...
repository overview, or the package itself to get to the package. The packages
are, as you can see, also grouped together by repository.

#important[
As of _May 21st, 2023_, ArchLinux migrated to `git` packaging and in the process
merged the `community` repository into `extra`. This change will also be
reflected in this document.

Read #linkfn("https://archlinux.org/news/git-migration-completed/")[the announcment] for
further information on how to proceed and update your system.
]

== Commands <commands>

Furthermore, I will denote a command execution in a shell with syntax
highlighting and a different background: #cmd[`uname -a`]. If a root shell is
needed, I will denote it with a preceding #emoji.lock: #cmd(root: true)[`touch /root/testfile`].

This then mostly includes any needed command line arguments.

== Console blocks

Multiple commands, where the execution user or rather the privilege needed, as
well as the current working directory, are of interest, are displayed in a
command execution block. You can infer the privilege the command was executed by
...
In case of a windows `cmd` command, you can easily distinguish it from the `linux` shell
commands by the filepath descriptor, which of course uses Windows syntax, as
well as a #emph("Window")s #emoji.window symbol:

#terminal(windows: true, "C:\\Program Files (x86)\Common Files")[`dir`]

== Files <files>

For the content of a file, a file listing is used, showing the content of the
file as well as the full filepath:
#filesrc("~/test.sh")[
```sh
#!/bin/bash
export var="value"
...
If the file is only partially displayed, or only an addition is shown, it is
denoted by three dots #sym.dots being appended to the file path:
...
In case the file is _readonly_, I will use the glasses symbol #emoji.glasses, if
you need elevated or specific permissions, the lock symbol #emoji.lock and if
the file needs to be executable, the joystick symbol #emoji.joystick:
...
If any term needs an explanation, I will explain it briefly in the following
form:
...
#note[
You have to configure #pkg("sample-package"), or rather #pkg-aur("sample-package-git"),
by editing #filepath("/etc/sample.conf") with e.g. #cmd[`nvim /etc/sample.conf`]
...
If there is someting unusual going on, which you don't understand, try turning
it off and on again.
...
If you've printed this guide, you might want to add some additional blank pages
for notes.